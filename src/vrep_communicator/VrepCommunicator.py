...
class Robot(thr.Thread):
    def __init__(self, handle, con_port):
    ...
    def get_angle_difference(self):
        goal_angle = self.vrep_con.get_object_orientation(self.handle, self.goal)
    ...
    def get_PID_impact(self, old_error, error_sum):
        desired_dir = self.vrep_con.get_object_orientation(self.handle, self.goal)
        ...
        u = up + ui + ud
        if u > 0:
            left_u = self.velocity - fabs(u)
            right_u = self.velocity
        else:
            left_u = self.velocity
            right_u = self.velocity - fabs(u)
    ...
    def get_robot_direction_vector(self):
        ...
        if dir_point_pos != ():
            ...
            raise Exception("dir_point_pos is Null")
            return ()
    ...
    def turn_to_a_point(self, point_pos):
        ...
        if angle_difference > 0:
            ...
            self.wheel_rotation(const.ROTATION_SPEED, -const.ROTATION_SPEED)
        while fabs(angle_difference) > 3:
            angle_difference = self.get_angle_difference(point_pos)
        self.stop()

    def run(self):
        self.vrep_con.create_dummy(self.goal)
        if fabs(self.get_angle_difference(self.goal)) > 3:
            self.turn_to_a_point(self.goal)
        old_error = 0
        error_sum = 0
        rate = rospy.Rate(10)
        while not rospy.is_shutdown() and self.get_robot_position().get_distance_to(self.goal) > 0.05:
            left_velo, right_velo, old_error, error_sum = self.get_PID_impact(old_error, error_sum)
            self.wheel_rotation(left_velo, right_velo)
            rate.sleep()
        self.stop()
        rate.sleep()
